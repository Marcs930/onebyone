...
define( 'FUDOU_VERSION', '6.1.3' );


/*  Copyright 2023 nendeb (email : nendeb@gmail.com )
...
/**
 * Block Initializer. ver5.8.1 v6.1.0
 */
require_once( 'block/fudou_block_init.php' );
 ...
 * 物件詳細画像 ver6.1.0
 */
require_once 'inc/inc-single_fudou_img.php';
...
/*
 * It is also possible to globally disable caching.
 *
 * @package WordPress6.1
 * v6.1.0
 */
if ( !function_exists( 'fudou_disable_caching' ) ) {
	function fudou_disable_caching( $wp_query ) {
		$wp_query->query_vars['cache_results'] = false;
	}
	add_action( 'parse_query', 'fudou_disable_caching' );
}
...
function fudou_version_check_warnings() {
	...
	//WordPressバージョンチェック
	if ( version_compare( floatval( $GLOBALS['wp_version'] ), '6.1', '<' ) ) {
		...
		//WordPress5.3 Fix
		if ( version_compare( floatval( $GLOBALS['wp_version'] ), '5.3', '>' ) ) {
		...
		//WordPress v6.1～
		if ( defined( 'FUDOU_FAVORITES_VERSION' ) )		{	if ( version_compare( FUDOU_FAVORITES_VERSION , '6.1.0', '<') )			{ $fudou_version_check_err .= ' 不動産お気に入りプラグイン を ver6.1.0以降 にバージョンアップしてください。(現在:'	. FUDOU_FAVORITES_VERSION . ')<br />' ; } }
		if ( defined( 'FUDOU_TWEETOLDPOST_VERSION' ) )		{	if ( version_compare( FUDOU_TWEETOLDPOST_VERSION , '6.1.0', '<') )		{ $fudou_version_check_err .= ' 不動産 Tweet Old Post プラグイン を ver6.1.0以降 にバージョンアップしてください。(現在:'	. FUDOU_TWEETOLDPOST_VERSION . ')<br />' ; } }
		if ( defined( 'FUDOU_TOKYO23_AREA_MAP_VERSION' ) )	{	if ( version_compare( FUDOU_TOKYO23_AREA_MAP_VERSION , '6.1.0', '<') )		{ $fudou_version_check_err .= ' 不動産東京23区エリアマッププラグイン を ver6.1.0以降 にバージョンアップしてください。(現在:'	. FUDOU_TOKYO23_AREA_MAP_VERSION . ')<br />' ; } }
		if ( defined( 'FUDOU_ZENKOKU_TODOUFKEN_MAP_VERSION' ) )	{	if ( version_compare( FUDOU_ZENKOKU_TODOUFKEN_MAP_VERSION , '6.1.0', '<') )	{ $fudou_version_check_err .= ' 不動産全国都道府県プラグイン を ver6.1.0以降 にバージョンアップしてください。(現在:'	. FUDOU_ZENKOKU_TODOUFKEN_MAP_VERSION . ')<br />' ; } }
		if ( defined( 'FUDOU_OGP_VERSION' ) )			{	if ( version_compare( FUDOU_OGP_VERSION , '6.1.0', '<') )			{ $fudou_version_check_err .= ' 不動産 OGP and TwitterCards プラグイン を ver6.1.0以降 にバージョンアップしてください。(現在:'	. FUDOU_OGP_VERSION . ')<br />' ; } }
		if ( defined( 'FUDOU_KOUKU_VERSION' ) )			{	if ( version_compare( FUDOU_KOUKU_VERSION , '6.1.0', '<') ) 			{ $fudou_version_check_err .= ' 不動産校区プラグイン を ver6.1.0以降 にバージョンアップしてください。(現在:'		. FUDOU_KOUKU_VERSION . ')<br />' ; } }
		if ( defined( 'FUDOU_INJOKEN_RIM_VERSION' ) )		{	if ( version_compare( FUDOU_INJOKEN_RIM_VERSION , '6.1.0', '<') )		{ $fudou_version_check_err .= ' 不動産イン条件利回りプラグイン を ver6.1.0以降 にバージョンアップしてください。(現在:'	. FUDOU_INJOKEN_RIM_VERSION . ')<br />' ; } }
		if ( defined( 'FUDOU_MAIL_VERSION' ) )			{	if ( version_compare( FUDOU_MAIL_VERSION , '6.1.0', '<') )			{ $fudou_version_check_err .= ' 不動産マッチングメールプラグイン を ver6.1.0以降 にバージョンアップしてください。(現在:'	. FUDOU_MAIL_VERSION . ')<br />' ; } }
		if ( defined( 'FUDOU_CUSTOM_JW_VERSION' ) )		{	if ( version_compare( FUDOU_CUSTOM_JW_VERSION , '6.1.0', '<') )			{ $fudou_version_check_err .= ' 不動産カスタマイズ条件検索プラグイン を ver6.1.0以降 にバージョンアップしてください。(現在:'	. FUDOU_CUSTOM_JW_VERSION . ')<br />' ; } }
		if ( defined( 'FUDOU_SHORT_CODE_VERSION' ) )		{	if ( version_compare( FUDOU_SHORT_CODE_VERSION , '6.1.0', '<') )		{ $fudou_version_check_err .= ' 不動産物件ショートコードプラグイン を ver6.1.0以降 にバージョンアップしてください。(現在:'	. FUDOU_SHORT_CODE_VERSION . ')<br />' ; } }
		if ( defined( 'FUDOU_BLOCK_SLIDER_VERSION' ) )		{	if ( version_compare( FUDOU_BLOCK_SLIDER_VERSION , '6.1.0', '<') )		{ $fudou_version_check_err .= ' 不動産ブロックスライダープラグイン を ver6.1.0以降 にバージョンアップしてください。(現在:'	. FUDOU_BLOCK_SLIDER_VERSION . ')<br />' ; } }
		if ( defined( 'FUDOU_KAIIN_VERSION' ) )			{	if ( version_compare( FUDOU_KAIIN_VERSION , '6.1.0', '<') ) 			{ $fudou_version_check_err .= ' 不動産会員プラグイン を ver6.1.0以降 にバージョンアップしてください。(現在:'	. FUDOU_KAIIN_VERSION . ')<br />' ; } }
		if ( defined( 'FUDOU_KAIIN_VIP_VERSION' ) )		{	if ( version_compare( FUDOU_KAIIN_VIP_VERSION , '6.1.0', '<') ) 		{ $fudou_version_check_err .= ' 不動産会員VIPプラグイン を ver6.1.0以降 にバージョンアップしてください。(現在:'	. FUDOU_KAIIN_VIP_VERSION . ')<br />' ; } }

		//ブロックテーマの場合 v6.1.1
		if( wp_is_block_theme() ){
			if ( defined( 'FUDOU_MAP_VERSION' ) )		{	if ( version_compare( FUDOU_MAP_VERSION , '6.1.1', '<') )			{ $fudou_version_check_err .= ' 不動産マッププラグイン を ver6.1.1以降 にバージョンアップしてください。(現在:'	. FUDOU_MAP_VERSION . ')<br />' ; } }
		}else{
			if ( defined( 'FUDOU_MAP_VERSION' ) )		{	if ( version_compare( FUDOU_MAP_VERSION , '6.1.0', '<') )			{ $fudou_version_check_err .= ' 不動産マッププラグイン を ver6.1.0以降 にバージョンアップしてください。(現在:'	. FUDOU_MAP_VERSION . ')<br />' ; } }
		}

		//OLDタイプ
		if ( defined( 'FUDOU_INJOKEN_CAT_VERSION' ) )		{	if ( version_compare( FUDOU_INJOKEN_CAT_VERSION , '5.9.0', '<') ) 		{ $fudou_version_check_err .= ' 不動産イン条件カテゴリプラグイン を ver5.9.0以降 にバージョンアップしてください。(現在:'. FUDOU_INJOKEN_CAT_VERSION . ')<br />' ; } }
		if ( defined( 'FUDOU_INJOKEN_TSUBOTANKA_VERSION' ) )	{	if ( version_compare( FUDOU_INJOKEN_TSUBOTANKA_VERSION , '5.5.0', '<') ) 	{ $fudou_version_check_err .= ' 不動産イン条件坪単価プラグイン を ver5.5.0以降 にバージョンアップしてください。(現在:'	. FUDOU_INJOKEN_TSUBOTANKA_VERSION . ')<br />' ; } }
		if ( defined( 'FUDOU_CHOUMEI_VERSION' ) )		{	if ( version_compare( FUDOU_CHOUMEI_VERSION , '6.0.1', '<') ) 			{ $fudou_version_check_err .= ' 不動産町名検索プラグイン を ver6.0.1以降 にバージョンアップしてください。(現在:'	. FUDOU_CHOUMEI_VERSION . ')<br />' ; } }
		if ( defined( 'FUDOU_CUSTOM_MS_VERSION' ) )		{	if ( version_compare( FUDOU_CUSTOM_MS_VERSION , '5.5.0', '<') ) 		{ $fudou_version_check_err .= ' 不動産カスタマイズ面積検索プラグイン を ver5.5.0以降 にバージョンアップしてください。(現在:'	. FUDOU_CUSTOM_MS_VERSION . ')<br />' ; } }
		if ( defined( 'FUDOU_CSV_VERSION' ) ) 			{	if ( version_compare( FUDOU_CSV_VERSION , '6.0.2', '<') ) 			{ $fudou_version_check_err .= ' 不動産汎用CSVプラグイン を ver6.0.2以降 にバージョンアップしてください。(現在:'		. FUDOU_CSV_VERSION . ')<br />' ; } }
	}
...
function fudou_twentyfifteen_remove_css(){
	...
	if( $template_name == 'twentyfifteen' 
		|| $template_name == 'twentysixteen' 
		|| $template_name == 'twentyseventeen' 
		|| $template_name == 'twentynineteen' 
		|| $template_name == 'twentytwenty' 
	){
			...
			if($object->post_type == 'fudo'){
				...
				switch ( $template_name ) {
					...
					case "twentytwentythree" :
						$template = locate_template(array('../../plugins/fudou/themes/single-fudo2023.php', 'template-canvas.php'));
			...
			switch ( $template_name ) {
				...
				case "twentytwentytwo" :
					...
					*/
					...
					break;
				case "twentytwentythree" :
					$template = locate_template(array('../../plugins/fudou/themes/archive-fudo2023.php', 'template-canvas.php'));
	...
	if ( !is_multisite() ) {
		...
		switch ( $template_name ) {
			...
			case "twentytwentytwo" :
				...
				break;
			case "twentytwentythree" :
				wp_enqueue_style( 'twentytwentythree-style2023', $plugin_url .'themes/style2023.css' );
				wp_enqueue_style( 'twentytwentythree-corners2023', $plugin_url .'themes/corners2023.css' );
				break;
			default:
			//	wp_enqueue_style( 'twentytwelve-style2012', $plugin_url .'themes/style2012.css' );
			//	wp_enqueue_style( 'twentytwelve-corners2012', $plugin_url .'themes/corners2012.css' );
...
function get_post_type_top_template_fudou( $template = '' ) {
	...
	if ( is_home() ) {
		...
		switch ( $template_name ) {
			case "twentytwentythree" :
				$template = locate_template(array('../../plugins/fudou/themes/home2023.php', 'template-canvas.php'));
 ...
 * テーマバージョン表記
 *
 * Sample Theme Name: unpc-light
 * Sample Version: 5.9.1
 *
 * @since Fudousan Plugin 6.0.0
 */
function add_footer_tag_fudou_theme() {
	// style.css 読込
	$default_headers = array( 'theme_name' => 'Theme Name', 'version' => 'Version' );
	$headers = get_file_data( get_stylesheet_directory() . '/style.css', $default_headers );

	$theme_name = isset( $headers['theme_name'] )	? $headers['theme_name'] : '';
	$version    = isset( $headers['version'] )		? $headers['version'] : '';

	echo "<!-- " . $theme_name . " Ver." . $version . " -->\n";
}
add_filter( 'wp_footer', 'add_footer_tag_fudou_theme' );
...
if (!function_exists('checkurl_fudou')) {
	function checkurl_fudou( $url ){
		...
		//v6.1.0
		if( false !== strpos( $url, 'http://localhost' ) ){
			return true;
		}
...
/**
 * widget foreseeing check.
 *
 * ver6.0.0
 */
if ( !function_exists( 'foreseeing_check_init_fudou' ) ) {
	function foreseeing_check_init_fudou(){
		global $foreseeing_check;
		$foreseeing_check = 1;
	}
}
add_action( 'init', 'foreseeing_check_init_fudou' );
