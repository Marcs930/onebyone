...
function my_custom_jyoutai_print( $post_id ) {
	...
	if($jyoutai_data=="4")  $value_dat = '成約';
	if($jyoutai_data=="9")  $value_dat = '削除';

	//text
	if( $jyoutai_data !='' && !is_numeric( $jyoutai_data ) ) $value_dat = $jyoutai_data;

	$value_dat = apply_filters( 'jyoutai_print', $value_dat, $post_id );
	echo $value_dat;
}
...
function my_custom_tatemonokozo_print( $post_id ) {
	...
	if( $tatemonokozo_data=="1" ) 	$value_dat = '木造';
	if( $tatemonokozo_data=="2" ) 	$value_dat = 'ブロック';
	if( $tatemonokozo_data=="3" ) 	$value_dat = '鉄骨造';
	if( $tatemonokozo_data=="4" ) 	$value_dat = 'RC';
	if( $tatemonokozo_data=="5" ) 	$value_dat = 'SRC';
	if( $tatemonokozo_data=="6" ) 	$value_dat = 'PC';
	if( $tatemonokozo_data=="7" ) 	$value_dat = 'HPC';
	if( $tatemonokozo_data=="9" ) 	$value_dat = 'その他';
	if( $tatemonokozo_data=="10" ) 	$value_dat = '軽量鉄骨';
	if( $tatemonokozo_data=="11" ) 	$value_dat = 'ALC';
	if( $tatemonokozo_data=="12" ) 	$value_dat = '鉄筋ブロック';
	if( $tatemonokozo_data=="13" ) 	$value_dat = 'CFT(コンクリート充填鋼管)';
	//text
	if( $tatemonokozo_data !='' && !is_numeric( $tatemonokozo_data ) ) $value_dat = $tatemonokozo_data;

	$value_dat = apply_filters( 'tatemonokozo_print', $value_dat, $post_id );
	echo $value_dat;
}
...
function my_custom_tatemonohosiki_print( $post_id ) {
	...
	$tatemonohosiki_data = get_post_meta( $post_id,'tatemonohosiki', true );
	if( $tatemonohosiki_data=="1" )	$value_dat = '壁芯';
	if( $tatemonohosiki_data=="2" )	$value_dat = '内法';
	//text
	if( $tatemonohosiki_data !='' && !is_numeric( $tatemonohosiki_data ) ) $value_dat = $tatemonohosiki_data;

	$value_dat = apply_filters( 'tatemonohosiki_print', $value_dat, $post_id );
	echo $value_dat;
}
...
function my_custom_tatemonoshinchiku_print( $post_id ) {
	...
	$tatemonoshinchiku_data = get_post_meta( $post_id,'tatemonoshinchiku', true );
	if( $tatemonoshinchiku_data =="0" ) $value_dat = '中古';
	if( $tatemonoshinchiku_data =="1" ) $value_dat = '新築・未入居';
	//text
	if( $tatemonoshinchiku_data !='' && !is_numeric( $tatemonoshinchiku_data ) ) $value_dat = $tatemonoshinchiku_data;

	$value_dat = apply_filters( 'tatemonoshinchiku_print', $value_dat, $post_id );
	echo $value_dat;
}
...
function my_custom_kanrininn_print( $post_id ) {
	...
	if( $kanrininn_data=="1" )	$value_dat = '管理人常駐　';
	if( $kanrininn_data=="2" )	$value_dat = '管理人日勤　';
	if( $kanrininn_data=="3" )	$value_dat = '管理人巡回　';
	if( $kanrininn_data=="4" )	$value_dat = '管理人無　';
	if( $kanrininn_data=="5" )	$value_dat = '管理人非常駐　';
	//text
	if( $kanrininn_data !='' && !is_numeric($kanrininn_data) ) $value_dat = $kanrininn_data;

	$value_dat = apply_filters( 'kanrininn_print', $value_dat, $post_id );
	echo $value_dat;
}
...
function my_custom_kanrikeitai_print( $post_id ) {
	...
	if( $kanrikeitai_data=="1" )	$value_dat = '自主管理　';
	if( $kanrikeitai_data=="2" )	$value_dat = '一部委託　';
	if( $kanrikeitai_data=="3" )	$value_dat = '全部委託　';
	//text
	if( $kanrikeitai_data !='' && !is_numeric($kanrikeitai_data) ) $value_dat = $kanrikeitai_data;

	$value_dat = apply_filters( 'kanrikeitai_print', $value_dat, $post_id );
	echo $value_dat;
}
...
function my_custom_kanrikumiai_print($post_id) {
	...
	if( $kanrikumiai_data=="1" )	$value_dat = '管理組合無';
	if( $kanrikumiai_data=="2" )	$value_dat = '管理組合有';
	//text
	if( $kanrikumiai_data !='' && !is_numeric($kanrikumiai_data) ) $value_dat = $kanrikumiai_data;

	$value_dat = apply_filters( 'kanrikumiai_print', $value_dat, $post_id );
	echo $value_dat;
}
...
function my_custom_heyamuki_print($post_id) {
	...
	$heyamuki_data = get_post_meta( $post_id,'heyamuki', true );

	if( $heyamuki_data=="1" ) 	$value_dat = '北';
	if( $heyamuki_data=="2" ) 	$value_dat = '北東';
	if( $heyamuki_data=="3" ) 	$value_dat = '東';
	if( $heyamuki_data=="4" ) 	$value_dat = '南東';
	if( $heyamuki_data=="5" ) 	$value_dat = '南';
	if( $heyamuki_data=="6" ) 	$value_dat = '南西';
	if( $heyamuki_data=="7" ) 	$value_dat = '西';
	if( $heyamuki_data=="8" ) 	$value_dat = '北西';
	//text
	if( $heyamuki_data !='' && !is_numeric( $heyamuki_data ) ) $value_dat = $heyamuki_data;

	$value_dat = apply_filters( 'heyamuki_print', $value_dat, $post_id );
	echo $value_dat;
}
...
function my_custom_targeturl_print($post_id) {
	...
	if($targeturl_data !=''){
		...
		echo '<a href="'.$targeturl_data.'" target="_blank"  rel="noopener noreferrer nofollow">'.$targeturl_data.'</a>';
	}
}
...
function my_custom_kakaku_print($post_id) {
	...
	if(get_post_meta($post_id,'kakakukoukai',true) == "0"){
		 ...
		 * Version: 5.8.1
		 **/
		echo apply_filters( 'fudou_kakaku_after', '', $post_id );


			// 金銭面	税額 ※ 単位：円
			$kakakuzei_data = get_post_meta( $post_id, 'kakakuzei', true );
		...
		/*
		 * fudou_kakaku_after2 Filter
		 * Version: 5.7.2
		 **/
		echo apply_filters( 'fudou_kakaku_after2', '', $post_id );
}
	...
	$kakakukoushin_data = get_post_meta($post_id,'kakakukoushin',true);
		if( $kakakukoushin_data == '0' ) {
			...
			if($kakakukoushin_data >= 100) {
				echo apply_filters( 'fudou_number_format', $kakakukoushin_data );
				echo "円";
			}else{
...
function my_custom_kakakuhoken_print($post_id) {
	...
	if($kakakuhoken_data == '9'){
		...
		if( is_numeric($kakakuhoken_data) && $kakakuhoken_data > 100  ){
			echo apply_filters( 'fudou_number_format', $kakakuhoken_data ) . '円　';
		}else{
			if( $kakakuhoken_data == '1' ){
				echo '要　';
			}
		}
	}
...
function my_custom_chushajo_print( $post_id ) {
	...
	if( get_post_meta( $post_id,'chushajoryokin',true ) ){
		echo apply_filters( 'fudou_number_format', get_post_meta( $post_id, 'chushajoryokin', true ) ) . '円　';
	}
	...
	if( $chushajokubun_data=="0" )	$value_dat = '';
	if( $chushajokubun_data=="1" )	$value_dat = '空有';
	if( $chushajokubun_data=="2" )	$value_dat = '空無';
	if( $chushajokubun_data=="3" )	$value_dat = '近隣';
	if( $chushajokubun_data=="4" )	$value_dat = '無';
	//text
	if( $chushajokubun_data !='' && !is_numeric( $chushajokubun_data ) ){
		$value_dat = $chushajokubun_data;
	}

	$value_dat = apply_filters( 'chushajokubun_print', $value_dat, $post_id );
	echo $value_dat;
...
function my_custom_nyukyogenkyo_print( $post_id ) {
	...
	if ( ( $bukkenshubetsu_data < 1200 && $bukkenshubetsu_data > 1000 ) || $bukkenshubetsu_data == 3212  ) {
		if( $nyukyogenkyo_data=="1" )	$value_dat = '更地';
		if( $nyukyogenkyo_data=="2" )	$value_dat = '古屋あり';
		if( $nyukyogenkyo_data=="10" )	$value_dat = '古屋あり(更地引渡可)';
		if( $nyukyogenkyo_data=="3" )	$value_dat = '賃貸中';
	}else{
		if( $nyukyogenkyo_data=="1" )	$value_dat = '居住中';
		if( $nyukyogenkyo_data=="2" )	$value_dat = '空家';
		if( $nyukyogenkyo_data=="3" )	$value_dat = '賃貸中';
		if( $nyukyogenkyo_data=="4" )	$value_dat = '未完成';
	}
	//text
	if( $nyukyogenkyo_data !='' && !is_numeric( $nyukyogenkyo_data ) ) $value_dat = $nyukyogenkyo_data;


	$value_dat = apply_filters( 'nyukyogenkyo_print', $value_dat, $post_id );
	echo $value_dat;
}
...
function my_custom_nyukyojiki_print( $post_id ) {
	...
	if( $nyukyojiki_data=="1" )	$value_dat = '即時 ';
	if( $nyukyojiki_data=="2" )	$value_dat = '相談 ';
	if( $nyukyojiki_data=="3" )	$value_dat = '期日指定 ';
	//text
	if( $nyukyojiki_data !='' && !is_numeric( $nyukyojiki_data ) ) $value_dat = $nyukyojiki_data.' ';

	$value_dat = apply_filters( 'nyukyojiki_print', $value_dat, $post_id );
	echo $value_dat;
}
...
function my_custom_nyukyosyun_print( $post_id ) {
	...
	if( $nyukyosyun_data=="1" )	$value_dat = '上旬';
	if( $nyukyosyun_data=="2" )	$value_dat = '中旬';
	if( $nyukyosyun_data=="3" )	$value_dat = '下旬';
	//text
	if( $nyukyosyun_data !='' && !is_numeric( $nyukyosyun_data ) ) $value_dat = $nyukyosyun_data.'';

	$value_dat = apply_filters( 'nyukyosyun_print', $value_dat, $post_id );
	echo $value_dat;
}
...
function my_custom_torihikitaiyo_print( $post_id ) {
	...
	if( get_post_meta($post_id,'bukkenshubetsu',true) < 3000 ) {
		if( $torihikitaiyo_data=="1" )	$value_dat = '売主';
	}else{
		if( $torihikitaiyo_data=="1" )	$value_dat = '貸主';
	}

	if( $torihikitaiyo_data=="2" )	$value_dat = '代理';
	if( $torihikitaiyo_data=="3" )	$value_dat = '専属';
	if( $torihikitaiyo_data=="4" )	$value_dat = '専任';
	if( $torihikitaiyo_data=="5" )	$value_dat = '一般';
	if( $torihikitaiyo_data=="6" )	$value_dat = '仲介';
	if( $torihikitaiyo_data=="9" )	$value_dat = 'その他';
	//text
	if( $torihikitaiyo_data !='' && !is_numeric( $torihikitaiyo_data ) ) $value_dat = $torihikitaiyo_data.'';

	$value_dat = apply_filters( 'torihikitaiyo_print', $value_dat, $post_id );
	echo $value_dat;
}
...
function my_custom_tochichimoku_print( $post_id ) {
	...
	if( $tochichimoku_data=="1" )	$value_dat = '宅地';
	if( $tochichimoku_data=="2" )	$value_dat = '田';
	if( $tochichimoku_data=="3" )	$value_dat = '畑';
	if( $tochichimoku_data=="4" )	$value_dat = '山林';
	if( $tochichimoku_data=="5" )	$value_dat = '雑種地';
	if( $tochichimoku_data=="9" )	$value_dat = 'その他';
	if( $tochichimoku_data=="10" )	$value_dat = '原野';
	if( $tochichimoku_data=="11" )	$value_dat = '田･畑';
	//text
	if( $tochichimoku_data !='' && !is_numeric( $tochichimoku_data ) ) $value_dat = $tochichimoku_data.'';

	$value_dat = apply_filters( 'tochichimoku_print', $value_dat, $post_id );
	echo $value_dat;
...
function my_custom_tochiyouto_print( $post_id ) {
	...
	if( $tochiyouto_data=="1" )	$value_dat = '第一種低層住居専用';
	if( $tochiyouto_data=="2" )	$value_dat = '第二種中高層住居専用';
	if( $tochiyouto_data=="3" )	$value_dat = '第二種住居';
	if( $tochiyouto_data=="4" )	$value_dat = '近隣商業';
	if( $tochiyouto_data=="5" )	$value_dat = '商業';
	if( $tochiyouto_data=="6" )	$value_dat = '準工業';
	if( $tochiyouto_data=="7" )	$value_dat = '工業';
	if( $tochiyouto_data=="8" )	$value_dat = '工業専用';
	if( $tochiyouto_data=="10" )	$value_dat = '第二種低層住居専用';
	if( $tochiyouto_data=="11" )	$value_dat = '第一種中高層住居専用';
	if( $tochiyouto_data=="12" )	$value_dat = '第一種住居';
	if( $tochiyouto_data=="13" )	$value_dat = '準住居';
	if( $tochiyouto_data=="99" )	$value_dat = '無指定';
	//text
	if( $tochiyouto_data !='' && !is_numeric( $tochiyouto_data ) ) $value_dat = $tochiyouto_data.'';

	$value_dat = apply_filters( 'tochiyouto_print', $value_dat, $post_id );
	echo $value_dat;
...
function my_custom_tochikeikaku_print( $post_id ) {
	...
	if( $tochikeikaku_data=="1" )	$value_dat = '市街化区域';
	if( $tochikeikaku_data=="2" )	$value_dat = '市街化調整区域';
	if( $tochikeikaku_data=="3" )	$value_dat = '非線引き区域';
	if( $tochikeikaku_data=="4" )	$value_dat = '都市計画区域外';
	//text
	if( $tochikeikaku_data !='' && !is_numeric( $tochikeikaku_data ) ) $value_dat = $tochikeikaku_data.'';

	$value_dat = apply_filters( 'tochikeikaku_print', $value_dat, $post_id );
	echo $value_dat;
}
...
function my_custom_tochichisei_print( $post_id ) {
	...
	if( $tochichisei_data=="1" )	$value_dat = '平坦';
	if( $tochichisei_data=="2" )	$value_dat = '高台';
	if( $tochichisei_data=="3" )	$value_dat = '低地';
	if( $tochichisei_data=="4" )	$value_dat = 'ひな段';
	if( $tochichisei_data=="5" )	$value_dat = '傾斜地';
	if( $tochichisei_data=="9" )	$value_dat = 'その他';
	//text
	if( $tochichisei_data !='' && !is_numeric( $tochichisei_data ) ) $value_dat = $tochichisei_data.'';

	$value_dat = apply_filters( 'tochichisei_print', $value_dat, $post_id );
	echo $value_dat;
}
...
function my_custom_tochisokutei_print( $post_id ) {
	...
	if( $tochisokutei_data=="1" )	$value_dat = '公簿';
	if( $tochisokutei_data=="2" )	$value_dat = '実測';
	//text
	if( $tochisokutei_data !='' && !is_numeric( $tochisokutei_data ) ) $value_dat = $tochisokutei_data.'';

	$value_dat = apply_filters( 'tochisokutei_print', $value_dat, $post_id );
	echo $value_dat;
}
...
function my_custom_tochisetback_print( $post_id ) {
	...
	if( $tochisetback_data=="1" )	$value_dat = '無';
	if( $tochisetback_data=="2" )	$value_dat = '有';
	//text
	if( $tochisetback_data !='' && !is_numeric( $tochisetback_data ) ) $value_dat = $tochisetback_data.'';

	$value_dat = apply_filters( 'tochisetback_print', $value_dat, $post_id );
	echo $value_dat;
}
...
function my_custom_tochisetsudo_print( $post_id ) {
	...
	if( $tochisetsudo_data=="1" )	$value_dat = '一方';
	if( $tochisetsudo_data=="2" )	$value_dat = '角地';
	if( $tochisetsudo_data=="3" )	$value_dat = '三方';
	if( $tochisetsudo_data=="4" )	$value_dat = '四方';
	if( $tochisetsudo_data=="5" )	$value_dat = '二方(除角地)';
	if( $tochisetsudo_data=="10" )	$value_dat = '接道なし';
	//text
	if( $tochisetsudo_data !='' && !is_numeric( $tochisetsudo_data ) ) $value_dat = $tochisetsudo_data.'';

	$value_dat = apply_filters( 'tochisetsudo_print', $value_dat, $post_id );
	echo $value_dat;
}
...
function my_custom_tochisetsudohouko1_print( $post_id ) {
	...
	if( $tochisetsudohouko1_data=="1" )	$value_dat = '北';
	if( $tochisetsudohouko1_data=="2" )	$value_dat = '北東';
	if( $tochisetsudohouko1_data=="3" )	$value_dat = '東';
	if( $tochisetsudohouko1_data=="4" )	$value_dat = '南東';
	if( $tochisetsudohouko1_data=="5" )	$value_dat = '南';
	if( $tochisetsudohouko1_data=="6" )	$value_dat = '南西';
	if( $tochisetsudohouko1_data=="7" )	$value_dat = '西';
	if( $tochisetsudohouko1_data=="8" )	$value_dat = '北西';
	//text
	if( $tochisetsudohouko1_data !='' && !is_numeric( $tochisetsudohouko1_data ) ) $value_dat = $tochisetsudohouko1_data.'';

	$value_dat = apply_filters( 'tochisetsudohouko1_print', $value_dat, $post_id );
	echo $value_dat;
}
...
function my_custom_tochisetsudoshurui1_print( $post_id ) {
	...
	if( $tochisetsudoshurui1_data=="1" )	$value_dat = '公道';
	if( $tochisetsudoshurui1_data=="2" )	$value_dat = '私道';
	//text
	if( $tochisetsudoshurui1_data !='' && !is_numeric( $tochisetsudoshurui1_data ) ) $value_dat = $tochisetsudoshurui1_data.'';

	$value_dat = apply_filters( 'tochisetsudoshurui1_print', $value_dat, $post_id );
	echo $value_dat;
}
...
function my_custom_tochisetsudoichishitei1_print( $post_id ) {
	...
	if( $tochisetsudoichishitei1_data=="1" )	$value_dat = '位置指定道路';
	//text
	if( $tochisetsudoichishitei1_data !='' && !is_numeric( $tochisetsudoichishitei1_data ) ) $value_dat = $tochisetsudoichishitei1_data .'';

	$value_dat = apply_filters( 'tochisetsudoichishitei1_print', $value_dat, $post_id );
	echo $value_dat;
}
...
function my_custom_tochisetsudohouko2_print( $post_id ) {
	...
	if( $tochisetsudohouko2_data=="1" )	$value_dat = '北';
	if( $tochisetsudohouko2_data=="2" )	$value_dat = '北東';
	if( $tochisetsudohouko2_data=="3" )	$value_dat = '東';
	if( $tochisetsudohouko2_data=="4" )	$value_dat = '南東';
	if( $tochisetsudohouko2_data=="5" )	$value_dat = '南';
	if( $tochisetsudohouko2_data=="6" )	$value_dat = '南西';
	if( $tochisetsudohouko2_data=="7" )	$value_dat = '西';
	if( $tochisetsudohouko2_data=="8" )	$value_dat = '北西';
	//text
	if( $tochisetsudohouko2_data !='' && !is_numeric( $tochisetsudohouko2_data ) ) $value_dat = $tochisetsudohouko2_data.'';

	$value_dat = apply_filters( 'tochisetsudohouko2_print', $value_dat, $post_id );
	echo $value_dat;
}
...
function my_custom_tochisetsudoshurui2_print( $post_id ) {
	...
	if( $tochisetsudoshurui2_data=="1" )	$value_dat = '公道';
	if( $tochisetsudoshurui2_data=="2" )	$value_dat = '私道';
	//text
	if( $tochisetsudoshurui2_data !='' && !is_numeric( $tochisetsudoshurui2_data ) ) $value_dat = $tochisetsudoshurui2_data.'';

	$value_dat = apply_filters( 'tochisetsudoshurui2_print', $value_dat, $post_id );
	echo $value_dat;
}
...
function my_custom_tochisetsudoichishitei2_print( $post_id ) {
	...
	if( $tochisetsudoichishitei2_data=="1" )	$value_dat = '位置指定道路';
	//text
	if( $tochisetsudoichishitei2_data !='' && !is_numeric( $tochisetsudoichishitei2_data ) ) $value_dat = $tochisetsudoichishitei2_data .'';

	$value_dat = apply_filters( 'tochisetsudoichishitei2_print', $value_dat, $post_id );
	echo $value_dat;
}
...
function my_custom_tochikenri_print( $post_id ) {
	...
	if( $tochikenri_data=="1" )	$value_dat = '所有権';
	if( $tochikenri_data=="2" )	$value_dat = '旧法地上';
	if( $tochikenri_data=="3" )	$value_dat = '旧法賃借';
	if( $tochikenri_data=="4" )	$value_dat = '普通地上';
	if( $tochikenri_data=="5" )	$value_dat = '定期地上';
	if( $tochikenri_data=="6" )	$value_dat = '普通賃借';
	if( $tochikenri_data=="7" )	$value_dat = '定期賃借';
	if( $tochikenri_data=="8" )	$value_dat = '一時使用';
	if( $tochikenri_data=="21" )	$value_dat = '地上権';
	if( $tochikenri_data=="22" )	$value_dat = '定期借地';
	if( $tochikenri_data=="23" )	$value_dat = '賃借権';
	if( $tochikenri_data=="99" )	$value_dat = 'その他';
	//text
	if( $tochikenri_data !='' && !is_numeric( $tochikenri_data ) ) $value_dat = $tochikenri_data.'';

	$value_dat = apply_filters( 'tochikenri_print', $value_dat, $post_id );
	echo $value_dat;
}
...
function my_custom_tochikokudohou_print( $post_id ) {
	...
	if( $tochikokudohou_data=="1" )	$value_dat = '要';
	if( $tochikokudohou_data=="2" )	$value_dat = '届出中';
	if( $tochikokudohou_data=="3" )	$value_dat = '不要';
	//text
	if( $tochikokudohou_data !='' && !is_numeric( $tochikokudohou_data ) ) $value_dat = $tochikokudohou_data.'';

	$value_dat = apply_filters( 'tochikokudohou_print', $value_dat, $post_id );
	echo $value_dat;
}
...
function my_custom_shozaichi_eki_map( $post_id, $latlng = '' ) {
	 ...
	 *
	 * @since Fudousan Plugin 1.9.3
	 * For inc-single-fudo.php $value = apply_filters( 'fudou_shozaichi_eki_map', $value, $geo_txt, $post_id, $latlng );
	 *
	 * @param str $value.
	 * @param int $post_id.
	 * @param str $latlng. '' or 'lat' or 'lng'.
	 * @return str 駅名又は座標.
	*/
	$value = apply_filters( 'fudou_shozaichi_eki_map', $value, $geo_txt, $post_id, $latlng );
...
function fudou_single_googlemaps( $post_id , $kaiin , $kaiin2 , $title) {
	...
	if ( $lat && $lng ){
		...
		function add_inc_single_fudo_js() {
			...
			wp_enqueue_script( 'single-fudo-fudoumap', $js_url, array(), '5.9.0', true );
		}
		 ...
		 * Version: 5.2.3
		 **/
		$fudoimg_data = apply_filters( 'pre_fudoimg_data', $fudoimg_data, $post_id, 'fudo_postmap', false );
			...
			if ( checkurl_fudou( $fudoimg_data )) {
				...
				if( $attachmentid !='' ){
					...
					if ( checkurl_fudou( $fudoimg_data )) {
						...
						$j_img =  ''.plugins_url().'/fudou/img/nowprinting.jpg';
					}
				}
			}
		...
		<script type="text/javascript"> 
			...
			var bukken_content = '<table class="gmapballoon"><tr>'
					...
					+ '<?php my_custom_shozaichi_print($post_id); ?><?php echo esc_html( get_post_meta($post_id, 'shozaichimeisho', true) ); ?><br />'
			...
			//marker 5.2.0
			var gmapmark    = '<?php echo plugins_url();?>/fudou/img/<?php echo $icon_url;?>';
			var gmapmark_b0 = '<?php echo plugins_url();?>/fudou/img/bus_r.png';
			var gmapmark_b1 = '<?php echo plugins_url();?>/fudou/img/bus_o.png';
			var gmapmark_b2 = '<?php echo plugins_url();?>/fudou/img/bus_b.png';
			var gmapmark_gs1 = '<?php echo plugins_url();?>/fudoukouku/img/gmapmark_pschool1.png';
			var gmapmark_gc1 = '<?php echo plugins_url();?>/fudoukouku/img/gmapmark_jhschool1.png';
			var gmapmark_gs2 = '<?php echo plugins_url();?>/fudoukouku/img/gmapmark_pschool2.png';
			var gmapmark_gc2 = '<?php echo plugins_url();?>/fudoukouku/img/gmapmark_jhschool2.png';
		...
		// Pre fudou Minify
		$buffer = apply_filters( 'pre_fudou_slider_minify_buffer', $buffer,  'single_fudou_map' );

		// fudou Minify
		$buffer = apply_filters( 'fudou_minify_buffer', $buffer, 'single_fudou_map' );
